      ...
      <ul>
        <%= for {deck, deck_stats} <- @decks do %>
          <li>
            ...
            Due: <%= deck_stats.due %> )
            <%= live_patch "Cards", to: Routes.card_index_path(@socket, :index, %{deck_id: deck.id}) %>
            <%=  live_patch "Review", to: Routes.review_path(@socket, :home, %{deck: deck}) %>
            <button class="button" phx-click="add-new-batch-of-learn-cards" phx-value-deck_id={ deck.id }> Add New Batch of Cards to Learn </button>
          </li>
        <% end %>
      </ul>
    </div>
