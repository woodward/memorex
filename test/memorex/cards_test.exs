defmodule Memorex.CardsTest do
  ...
  describe "update_card_when_reviewing!" do
    test "updates the card and also the due field, and incrementes the reps field" do
      ...
      updated_card = Cards.update_card_when_reviewing!(card, %{interval: Duration.parse!("P10D")}, time_now)
  ...
  describe "create_from_note/1" do
    test "can create bidirectional cards from a note" do
      note = Note.new(content: ["one", "two"], bidirectional?: true) |> Repo.insert!()
    ...
    test "can create unidirectional cards from a note" do
      note = Note.new(content: ["one", "two"], bidirectional?: false) |> Repo.insert!()
      ...
      assert card.note_question_index == 0
      assert card.note_answer_index == 1
  ...
  describe "get_one_random_due_card/2" do
    test "returns one random due card for a deck (only active cards)" do
      ...
      deck1 = Repo.insert!(%Deck{})
      note1 = Repo.insert!(%Note{deck: deck1, content: ["First", "Second"]})
      card1 = Repo.insert!(%Card{note: note1, due: due, card_status: :active})
      _card2_suspended = Repo.insert!(%Card{note: note1, due: due, card_status: :suspended})
      _card3_buried = Repo.insert!(%Card{note: note1, due: due, card_status: :buried})
